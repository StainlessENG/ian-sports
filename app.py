...
out.append(f”URL: http://{request.host}/player_api.php?username={u}&password={VALID_USERS[u]}\n”)
...
u = check_auth()
if not u:
return jsonify({“user_info”: {“auth”: 0, “status”: “Disabled”, “message”: “Invalid credentials”}}), 401
...
if act == "get_live_streams":
    try:
        ...
        if cat_id:
            chans = [c for c in chans if c["category_id"] == cat_id]
        
        # Build proper stream URLs
        result = []
        for c in chans:
            result.append({
                "num": c["num"],
