...
# ---- Config (hardcoded to your m3u4u links) ----
...
def _refresh_epg(force=False):
    now = time.time()
    if not force and _epg_cache["content"] and (now - _epg_cache["fetched_at"] < CACHE_TTL):
        return
    r = _http_get(EPG_URL, timeout=30)
    _epg_cache["content"] = r.content
...
def _user_info(username):
    return {
        ...
        "exp_date": "1780185600",
        "active_cons": "0",
        "allowed_output_formats": ["m3u8", "ts"],  # both
    }

# =========================
# 1) /player_api.php  (Xtream API)
# =========================
    ...
    if action == "get_live_streams":
        ...
        for ch in _cache["channels"]:
            streams.append({
                ...
                "tvg_id": ch["tvg_id"],
                "custom_sid": "",
                "direct_source": ch["url"],  # direct to m3u4u
            })
...
def get_php():
    ...
    _ = request.args.get("output", "m3u8")  # accepted; we serve direct URLs
...
# =========================
# 3) /live/<username>/<password>/<stream_id>  (redirect to source)
# =========================
...
# =========================
# + /xmltv.php  (EPG proxy with 30-min cache)
# =========================
@app.route("/xmltv.php")
def xmltv():
    try:
        _refresh_epg()
        return Response(_epg_cache["content"], mimetype="application/xml")
