...
# PROXY MODE: Set to True only if you have bandwidth to spare

# False = redirect (lower server load, may pause on some players)

# True = proxy (higher server load, more stable playback)
...
@app.route(”/”)
def index():
mode = “PROXY MODE” if PROXY_STREAMS else “REDIRECT MODE”
return f”✅ Xtream Bridge running OK ({mode})”
...
def live_stream(username, password, stream_id, ext):
if not valid_user(username, password):
return Response(“Invalid credentials”, status=403)
...
for s in data["streams"]:
    if s["stream_id"] == stream_id:
        source_url = s["direct_source"]
        break

if not source_url:
    return Response("Stream not found", status=404)

# PROXY MODE: Full stream proxying (high bandwidth usage)
if PROXY_STREAMS:
    try:
        resp = requests.get(source_url, stream=True, timeout=10, headers={
...
# REDIRECT MODE: Low bandwidth, but may pause on some players
else:
    return redirect(source_url, code=307)  # 307 = Temporary Redirect (preserves method)
```
